...
#df = df.sort_values(by=['MOS'], inplace=True)
#df = df['MOS'].replace(to_replace = "inf", value = 0)
#df = df.sort_index(inplace=True)
#dict0inf = {'MOS':{'inf': "0"}}
#df = df.replace(dict0inf)
#sum(df['MOS'])
#df = df[['MOS']].replace('inf', 0, inplace=True)
#df.sort_values(by='MOS', ascending=False)
#mos_list = [0 if x=='inf' else x for x in mos_list]
#mos_list[-1] = 0
